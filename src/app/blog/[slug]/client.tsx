...
import { SanityContent } from "@/components/blog/sanity-content";
  ...
  return (
    <>
      {content.split("\n").map((line, i) => {
        ...
        return <p key={i} className="leading-relaxed text-muted-foreground mb-4">{line}</p>;
      })}
    </>
  );
}
...
export function BlogDetailClient({ post }: Props) {
  const isSanityContent = Array.isArray(post.body);
